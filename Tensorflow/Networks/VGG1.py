...
def model_creation(isAConnect=False,Wstd=0,Bstd=0,
        ...
        if(not(isAConnect)):
                model = tf.keras.models.Sequential([
                        ...
                        RandomZoom(0.2),
                        Conv2D(filters=64,kernel_size=(3,3),strides=(1,1),activation='relu',padding="same",kernel_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        Conv2D(filters=64,kernel_size=(3,3),strides=(1,1),activation='relu',padding="same",kernel_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                ...
                model = tf.keras.models.Sequential([
                        ...
                        RandomTranslation(0.1,0.1),
                        #RandomZoom(0.2),
                        Conv_AConnect(filters=64,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        ReLU(),
                        Conv_AConnect(filters=64,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        ...
                        ReLU(),
                        MaxPool2D(pool_size=(3,3),strides=(2,2),padding="SAME"),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",Op=1,isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        ReLU(),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",Op=1,isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        ...
                        Flatten(),
                        #Dropout(0.1),
                        FC_AConnect(256,Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=FC_pool,isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(0.0005)),
                        BatchNormalization(),
                        ReLU(),
                        #Dropout(0.1),
