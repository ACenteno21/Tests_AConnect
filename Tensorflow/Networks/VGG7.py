...
def model_creation(isAConnect=False,Wstd=0,Bstd=0,
        ...
        if(not(isAConnect)):
                ...
                model = tf.keras.models.Sequential([
                        ...
                        RandomTranslation(0.1,0.1),
                        RandomZoom(0.2),
                        Conv_AConnect(filters=128,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        Conv_AConnect(filters=128,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        MaxPool2D(pool_size=(2,2),strides=(2,2),padding="SAME"),
                        Conv_AConnect(filters=256,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        Conv_AConnect(filters=256,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        MaxPool2D(pool_size=(2,2),strides=(2,2),padding="SAME"),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        ...
                        Flatten(),
                        #Dropout(0.1),
                        FC_AConnect(1024,Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=FC_pool,isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        ReLU(),
                        BatchNormalization(),
                        #Dropout(0.1),
                        FC_AConnect(10,Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=FC_pool,isQuant=isQuant,bw=bw,weights_regularizer = tf.keras.regularizers.l2(1e-6)),
                        Softmax()
            ])
